...
const Equipment = () => {
  ...
  const addEquipments = useStore((s) => s.addEquipments);
  ...
  const handleSingleSubmit = (e) => {
    ...
    const equipment = {
      id: `eq-${Date.now()}`,
      code: newEq.code.trim().toUpperCase(),
  ...
  const handleParseBulk = () => {
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      // Support both commas and tabs as delimiters
      let parts = [];
      if (line.includes('\t')) {
        parts = line.split('\t');
      } else {
  ...
  const handleImportBulkSubmit = () => {
    // Check duplicates in import rows
    ...
    if (hasInternalDuplicates) {
      alert('Error: There are duplicate equipment codes within your bulk import list. Every equipment must have a unique code.');
      return;
    }

    // Check duplicates against existing system database
    ...
    if (existingDuplicates.length > 0) {
      alert(`Error: The following equipment codes already exist in the database: ${existingDuplicates.map((r) => r.code).join(', ')}. Please edit them to be unique.`);
      return;
    }

    const itemsToImport = bulkRows.map((row) => ({
      id: `eq-imported-${Date.now()}-${Math.random().toString(36).substr(2, 9)}`,
