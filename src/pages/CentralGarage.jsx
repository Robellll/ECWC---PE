...
const CentralGarage = () => {
  ...
  const vehicles       = useStore((s) => s.garageVehicles);
  ...
  // Keep drawer in sync when store changes (e.g. stage advance)
  const selectedFromStore = vehicles.find((v) => v.id === selectedVehicle?.id);
  const drawerVehicle = selectedFromStore || selectedVehicle;
  ...
  const handleAddSubmit = (e) => {
    ...
    const vehicle = {
      ...
      reportedIssue: newVehicle.reportedIssue,
      managerNotes: '',
      technician: newVehicle.technician,
      ...
      progressLog: [{
        id: `l${Date.now()}`,
        text: `Vehicle received at Central Garage. Initial registration complete.`,
        ...
        <table className="garage-table">
          ...
          <tbody>
            {filteredVehicles.length === 0 ? (
              ...
              >
                <td>
                  <span className={`priority-badge table-priority ${priorityClass(v.priority)}`}>
                    {v.priority}
                  </span>
                </td>
                ...
                <td>{v.model}</td>
                <td>
                  <span className="stage-pill">{v.stage}</span>
                </td>
                <td className="text-muted">
                  {new Date(v.registeredDate).toLocaleDateString('en-GB', { day: '2-digit', month: 'short', year: 'numeric' })}
                </td>
                <td>
                  <DurationCell startTime={v.registeredDate} endTime={v.completedDate} />
                </td>
                <td>
                  <span className={`status-badge ${v.status === 'Completed' ? 'success' : 'warning'}`}>
                    {v.status}
                  </span>
                </td>
                <td>{v.technician}</td>
          ...
          <div className="modal-content">
            ...
            <form onSubmit={handleAddSubmit}>
              ...
              <div className="form-group">
                ...
                <textarea
                  required
                  value={newVehicle.reportedIssue}
                  onChange={(e) => setNewVehicle({ ...newVehicle, reportedIssue: e.target.value })}
                  rows="3"
                  placeholder="Describe the problem reported by the operator or driver…"
                />
              </div>
              <div className="form-group">
                ...
                <input
                  required
                  type="text"
                  value={newVehicle.technician}
                  onChange={(e) => setNewVehicle({ ...newVehicle, technician: e.target.value })}
                  placeholder="e.g. Dawit Y."
                />
              </div>
