...
const empty = () => ({
  ...
  plantId: '', materialId: '', quantityProduced: '', unit: 'm³',
  shift: 'day', operatorName: '', remarks: '', projectId: '',
});
...
export default function ProductionDaily() {
  ...
  const handleSubmit = async (e) => {
    ...
    setError('');
    try {
      const payload = { ...form, quantityProduced: Number(form.quantityProduced), projectId: form.projectId || undefined };
  ...
  return (
    <ProductionShell title="Daily Production" subtitle="Record daily output — stock updates automatically"
    ...
    >
      ...
      <ProductionDataTable columns={columns} rows={displayedRows} searchKeys={['plantName', 'materialName', 'operatorName']} canEdit={isProductionEditor}
        onEdit={(r) => { setEditing(r.id); setForm({ productionDate: r.productionDate?.slice?.(0, 10) || r.productionDate, plantId: r.plantId, materialId: r.materialId, quantityProduced: r.quantityProduced, unit: r.unit, shift: r.shift, operatorName: r.operatorName, remarks: r.remarks, projectId: '' }); setOpen(true); }}
      ...
      >
        ...
        <div className="production-form-grid">
          ...
          <FormField label="Plant"><select required value={form.plantId} onChange={(e) => setForm({ ...form, plantId: e.target.value })}><option value="">Select…</option>{plants.map((p) => <option key={p.id} value={p.id}>{p.name}</option>)}</select></FormField>
          ...
          <FormField label="Project (for demand sync)"><select value={form.projectId} onChange={(e) => setForm({ ...form, projectId: e.target.value })}><option value="">Auto from plant</option>{projects.map((p) => <option key={p.id} value={p.id}>{p.name}</option>)}</select></FormField>
