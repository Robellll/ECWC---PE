...
export async function fetchProductionDashboard() {
  ...
  const [
    ...
    sql`
      SELECT name, code, status_changed_at
      FROM prod_plants WHERE status = 'maintenance'
  ...
  const avgDaily = dailyTrend.length
    ...
    : 0;
  const activePlants = plantStats[0]?.active || 0;
  const totalPlants = plantStats[0]?.total || 0;
  const plantUtilization = totalPlants ? Math.round((activePlants / totalPlants) * 100) : 0;
  ...
  const alerts = [
    ...
    ...maintenancePlants.map((r) => ({
      type: 'maintenance',
      severity: 'info',
      message: `Under maintenance: ${r.name} (${r.code})`,
    })),
  ];
