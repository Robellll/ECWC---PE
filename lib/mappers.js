...
export function mapEquipment(row) {
  ...
  return {
    ...
    equipmentType: row.equipment_type_label || EQUIPMENT_TYPE_TO_UI[row.type] || row.type || '',
    make: row.make || '',
    manufacturingYear: row.manufacturing_year ?? null,
    fuelNorm: row.fuel_norm != null ? Number(row.fuel_norm) : null,
    leaseRateHour: row.lease_rate_hour != null ? Number(row.lease_rate_hour) : null,
